        ...
        namespace CodeInjection
        {
            ...
            internal class CodeInjectionManager
            {
                ...
                internal class InjectedPoint
                {
                    ...
                    internal List<ShaderProperty> shaderProperties = new List<ShaderProperty>();
                    ...
                    static InjectedPoint Deserialize(string strData, object[] args)
                    {
                        ...
                        Func<object, string, object> onDeserializeShaderPropertyList = (obj, data) =>
                        {
                            ...
                            var split = Serialization.SplitExcludingBlocks(data.Substring(5, data.Length - 6), ',',
                                true, true, "()", "[]");
                            foreach (var spData in split)
                            {
                                ...
                                var vars = Serialization.SplitExcludingBlocks(spData.Substring(3, spData.Length - 4),
                                    ';', true, true, "()", "[]");
                    ...
                    public InjectedPoint(string name, string displayName, ShaderProperty.ProgramType program,
                        InjectableBlock block)
                    {
                        this.name = name;
                        ...
                        foreach (var line in block.codeLines)
                        {
                            ...
                            foreach (var sp in shaderProperties)
                            {
                                ...
                                if (Regex.IsMatch(line, pattern))
                                {
                                    ...
                                    stringBuilder.AppendLine(indent + lineIndent +
                                                             sp.PrintVariableSampleDeferred(
                                                                 ShaderGenerator2.CurrentInput,
                ...
                internal class InjectedFile
                {
                    ...
                    internal List<InjectedPoint> injectedPoints = new List<InjectedPoint>();
                    ...
                    void OnDeserialize()
                    {
                        ...
                        if (!string.IsNullOrEmpty(guid))
                        {
                            ...
                            {
                                Debug.LogError("Can't find path for Code Injection file GUID: " + guid +
                                               " (filename: \"" + filename + "\")");
                                return;
                            }
                        ...
                        for (int i = injectedPoints.Count - 1; i >= 0; i--)
                        {
                            ...
                            if (matchingBlock == null)
                            {
                                Debug.LogError(string.Format(
                                    "Block wasn't found in source file. Block name: \"{0}\", Source file: \"{1}\"",
                                    ip.blockName, this.filename));
                                injectedPoints.RemoveAt(i);
                                return;
                            }
                                ...
                                while ((line = stringReader.ReadLine()) != null)
                                {
                                    ...
                                    if (line.StartsWith("//# "))
                                    {
                                        ...
                                        else if (trimmedLine.StartsWith("//# WITH:"))
                                        {
                                            ...
                                            if (!currentBlock.isReplaceBlock)
                                            {
                                                throw new System.Exception(
                                                    "'WITH:' tag only works with 'REPLACE:' blocks");
                                            }
                                        ...
                                        else if (trimmedLine.ToLowerInvariant().StartsWith("//# info:"))
                                        {
                                            ...
                                            {
                                                throw new System.Exception(
                                                    "'INFO:' tag only works with 'REPLACE:' blocks");
                                            }
                                        ...
                                        else if (line.StartsWith("//# float"))
                                        {
                                            ...
                                            switch (parts[1])
                                            {
                                                ...
                                                case "color_rgba":
                                                    ...
                                                    break;
                                                default:
                                                    throw new System.Exception("Invalid parsed property type: " +
                                                                               parts[1]);
                                            }
                    ...
                    internal void ShowGUI(Template template, float margin)
                    {
                        ...
                        System.Action parseNewFile = () =>
                        {
                            if (newIncludeFile != includeFile)
                            {
                                if (!TryParseIncludeFile(newIncludeFile, template))
                                {
                                    includeFile = null;
                                    Debug.LogError(
                                        ShaderGenerator2.ErrorMsg("Couldn't load code injection include file."));
                                }
                            }
                        };
                        ...
                        Action<int, float> drawInjectedPoint = (index, margin2) =>
                        {
                            ...
                            EditorGUILayout.BeginHorizontal();
                            {
                                ...
                                EditorGUI.BeginChangeCheck();
                                {
                                    ...
                                    var rect = GUILayoutUtility.GetRect(label, EditorStyles.foldout,
                                        GUILayout.ExpandWidth(true));
                                    Rect hoverRect = rect;
                                    rect.xMin += 4; // small left padding
                                    ...
                                    if (hasShaderProperties)
                                    {
                                        ...
                                        headersExpanded[point.displayName] =
                                            TCP2_GUI.HeaderFoldoutHighlightErrorGrayPosition(rect,
                                                headersExpanded[point.displayName], label, false, highlight);
                                    }
                                    else
                                    {
                        ...
                        if (replaceBlockCount == 0)
                        {
                            ...
                            {
                                GUILayout.Space(margin);
                                EditorGUILayout.HelpBox(string.Format("0 'REPLACE' block found", replaceBlockCount),
                                    MessageType.Info);
                            }
                            GUILayout.EndHorizontal();
                        }
                        else
                        {
                            ...
                            GUILayout.BeginHorizontal();
                            {
                                GUILayout.Space(margin);
                                EditorGUILayout.HelpBox(
                                    string.Format("{0} 'REPLACE' block{1} found:\n{2}", replaceBlockCount,
                                        replaceBlockCount > 1 ? "s" : "",
                                        sb.ToString().TrimEnd(Environment.NewLine.ToCharArray())), MessageType.Info);
                            }
                            GUILayout.EndHorizontal();
                        }
                    ...
                    void AddBlockAtInjectionPoint(Template.InjectionPoint injectionPoint, InjectableBlock block)
                    {
                        ...
                        injectedPoints.Add(new InjectedPoint(injectionPoint.name, displayName, injectionPoint.program,
                            block));
                ...
                List<InjectedFile> injectedFiles = new List<InjectedFile>();
